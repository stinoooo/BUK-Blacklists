...
class Developer(commands.Cog):
    ...
    @app_commands.command(name="kick_user", description="Kick a user from all servers (excluding appeals server).")
    @is_bot_developer()
        ...
        for guild in self.bot.guilds:
            if guild.id != 1236376514430500914:  # Exclude the appeals server
                try:
                    ...
                    pass
        await interaction.response.send_message(f"User {user} kicked from: {', '.join(banned_servers)}", ephemeral=True)
